...
async function loadPage(path) {
  ...
  if (!res.ok) { showEditor(path); return; }

  const text = await res.text();
  const bodyHtml = parse(text);

  const pageTitle = decodeURIComponent(path.replace('pages/', '').replace('.txt',''));
...
async function markMissingLinks() {
  ...
  for (const link of links) {
    ...
    try {
      const res = await fetch(path);
      if (!res.ok) { link.classList.add('missing'); link.title='未作成ページ'; }
    } catch { link.classList.add('missing'); }
  }
}
